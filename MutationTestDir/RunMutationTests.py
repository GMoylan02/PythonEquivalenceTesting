...
TARGET_PACKAGE = "sorted_containers_mutants"
ORIG_PACKAGE = "sorted_containers_orig"
    ...
    for module in modules:
        ...
        for class_name, info in class_mutant_info.items():
            for mutant_entry in info["mutants"]:
                ...
                killed, cov, time_to_kill = run_class_fuzz_case(module, class_name, mutant_entry, idx)
                if killed:
...
def _orig_module_name(mutated_module_name: str) -> str:
    """
    grab the original (non-mutated) module name from the mutated module name.
    """
    prefix = TARGET_PACKAGE + "."
    if mutated_module_name.startswith(prefix):
        return ORIG_PACKAGE + "." + mutated_module_name[len(prefix):]
    if mutated_module_name == TARGET_PACKAGE:
        return ORIG_PACKAGE
    return mutated_module_name


def run_class_fuzz_case(module, class_name, mutant_entry, idx):
    """
    Build and run a class equivalence test for a single mutant method

    The equivalence test is performed between
    1. The original class taken from the original module
    2. A mutant class which is just the original class, but one of its methods is replaced with the mutant
       being tested
    """
    log_file = "hypofuzz_failures.log"
    coverage_file = _coverage_file_path(idx)

    # TODO refactor this so that aliases reflect the mutated method so that logs are clearer
    # alias for the mutated module
    mutated_mod_alias = f"mod_{idx}"

    # module in the original package
    orig_mod_name = _orig_module_name(module.__name__)
    orig_mod_alias = f"orig_mod_{idx}"

    mutant_method = mutant_entry["method_name"]
    mutant_attr   = mutant_entry["attr_name"]

    mutant_func_accessor = (
        f'getattr(getattr({mutated_mod_alias}, "{class_name}"), "{mutant_attr}")'
    )
    cov_boilerplate = _coverage_boilerplate(mutant_func_accessor, coverage_file)
...
os.environ["MUTANT_UNDER_TEST"] = ""
sys.path.insert(0, r'{PROJECT_ROOT}')
import {module.__name__} as {mutated_mod_alias}
import {orig_mod_name} as {orig_mod_alias}
OrigClass_{idx} = getattr({orig_mod_alias}, "{class_name}")
OrigClass_{idx}.__name__ = "{class_name + str(idx)}"
MutantClass_{idx} = type(
    "{class_name}",
    (OrigClass_{idx},),
    {{"{mutant_method}": getattr(getattr({mutated_mod_alias}, "{class_name}"), "{mutant_attr}")}},
)
MutantClass_{idx}.__name__ = "{class_name + str(idx)}"
{cov_boilerplate}
{test_body}
"""
...
def get_class_mutants(module):
    ...
    for cls_name, cls_obj in inspect.getmembers(module, inspect.isclass):
        ...
        for attr_name, method_obj in inspect.getmembers(cls_obj):
            ...
            if orig_match and orig_match.group(1) == cls_name:
                orig_methods[orig_match.group(2)] = method_obj
                continue

            mut_match = cls_mutant_pattern.match(attr_name)
            if mut_match and mut_match.group(1) == cls_name:
                mutants.append({
